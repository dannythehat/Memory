# AIDY Blocker 1 — Aggregation Redesign Pre-Registration (v9 — MATHEMATICS + FIXTURE FROZEN, 2026-09-22)

**Status: v9 — the aggregation MATHEMATICS AND THE T1 SOURCE FIXTURE ARE APPROVED AND FROZEN.** NOT IMPLEMENTED. No production, config or holdout touched. The v8 execution gap is closed: one immutable production-shaped source state now drives both the current Build 20→21→22 path and the frozen replacement mathematics. Formal-forward and live-money authority remain OFF.
...
| **v9 (this)** | **Fixture freeze completed.** Real liquidity/reclaim event generated by the real builder; 70-subject histories rebuilt; Build-3 gate trust history carries packet/version/scope linkage; every commitment has explicit `decision_time_utc` and `correct` is derived from prediction vs realised class; current and replacement paths executed from one immutable state; one combined manifest digest frozen. |
...
### 0.3 Fixture freeze COMPLETE — v9

The v8 open items are now closed by execution, not assumption.

- **Real identities/history:** the fixture is built against the **70 connected real sub-calculator subjects** emitted by the genuine builders. Non-directional/context-only subjects remain present with zero fabricated directional history.
- **Build-3 trust shape:** gate history carries current gate/version linkage, historical packet digests, exact scope keys, decision time, resolution time, predicted class, realised class, score and derived correctness. Build 21's trust envelopes are regenerated from these rows using the real Build-3 functions.
- **PIT correctness:** every commitment contains explicit `decision_time_utc`; `correct` is generated as `int(predicted_class == realised_direction)` and re-validated before either path executes. All decisions precede resolution and all history resolves before the fixture `as_of`.
- **Real liquidity evidence:** the final five completed M1 bars contain a deterministic sweep/reclaim of the frozen prior-day low. The real Liquidity/Reclaim expert classifies it as `reclaim_retest_hold` and votes **bullish**. No vote is injected directly.
- **One-state dual-path execution:** the same reloaded fixture drives the current Build 20→21→22 path and the frozen replacement path.
- **Negative control:** current path → **ABSTAIN / insufficient_directional_authority**, `directional_total = 0.075024`.
- **Replacement reachability:** frozen replacement → **BULLISH**, `meta_balance = 1.000000`, with exactly two qualifying independent root families:
  - `price_action`: signed/strength **0.331592**
  - `liquidity_mechanism`: signed/strength **0.373330**
- **Positive real contributors:** `m5_price_structure_expert:m5_candle_pressure` reliability **0.439855**; `h4_price_structure_expert:h4_acceleration` **0.223329**; `liquidity_reclaim_expert:liquidity_prior_day_low` **0.373330**. All are derived from frozen prediction/outcome histories, not hand-set reliabilities.

Frozen canonical fixture:

- canonical size: **8,576,293 bytes/chars**
- SHA-256: `74e92fe4a7d38e803da8469262e9339e795d3b9d1f3d7c9594a599560c3e3c4e`
- combined manifest digest: `53e691f0e70308030beda0fd1101f0cd032b51875ecfe2c834aacff507150ba6`
- validated AIDY source SHA: `47ffe131b9a8d2180c8d78ba3c1dc1b9253b9e4a`
- disposable execution PR: AIDY #244, **never to be merged**
- Memory storage: exact raw UTF-8 bytes split into 12 ordered chunks under `projects/aidy/fixtures/blocker1_v9_full_fixture.parts/`; direct transfer verification confirmed every chunk equals the corresponding substring of the source Git blob. Reassembly/index: `projects/aidy/fixtures/blocker1_v9_full_fixture.index.json`.
- validation manifest: `projects/aidy/fixtures/blocker1_v9_validation_manifest.json`
- exact executed validators: `projects/aidy/fixtures/blocker1_v9_fixture_probe.py` and `projects/aidy/fixtures/blocker1_v9_full_fixture_validator.py`

**Fixture construction disclosure:** this is an engineering reachability fixture, not market-edge evidence. The history was deliberately constructed before implementation to test the frozen mathematics. It does not use the 41 live-cycle outcomes to tune any frozen threshold.
...
### 7.1 T1 — Production-realistic reachability, v9 immutable real-pipeline fixture

The withdrawn v7 eight-subject fixture is replaced by the v9 source state described in §0.3. The fixture is generated only by the real connected builders plus five explicit UNKNOWN packets. The connected manifest contains **70 real sub-calculator identities**. Both paths consume the same reloaded fixture bytes.

    OLD PATH: fixture -> 15 real/UNKNOWN packets -> Build 20 -> Build 21 -> Build 22
    NEW PATH: same fixture -> same packets -> real sub-calculators
              -> frozen Stage A reliability -> decision dependency graph
              -> root-family aggregation -> frozen meta rule

**Scenario A — bullish reachable, now proven by execution.** The real fixture contains a prior-day-low M1 sweep/reclaim which the real liquidity builder classifies as `reclaim_retest_hold`. Historical skill is positive only for three real currently-bullish sensors; bearish/neutral sensors are at or below their PIT baseline and therefore silent under §3.1. Executed result:

- current path: **ABSTAIN**, `directional_total = 0.075024`
- replacement: **BULLISH**, `meta_balance = 1.000000`
- `price_action = +0.331592`
- `liquidity_mechanism = +0.373330`
- qualifying family count = **2**

This is the required negative-control/positive-reachability proof. The replacement succeeds because reliability and independence are separated; the current path still multiplies redundancy into gate authority and remains below 0.30.

**Scenario B — bearish reachable.** Mirror the frozen directional predictions/votes while leaving topology, histories, timestamps and constants unchanged; expected **BEARISH**. This remains a deterministic transformation test, not a second fitted fixture.

**Scenario C — genuine independent disagreement.** Reverse the liquidity root's current directional evidence while retaining its strength; expected **ABSTAIN `genuine_independent_disagreement`**.

**Scenario D — insufficient independent families.** Force the liquidity contributor histories below `MIN_CONTRIBUTOR_N`; expected **ABSTAIN `insufficient_independent_families`**.

**Scenario E — no directional evidence.** Set every directional history at/below its PIT baseline; expected **ABSTAIN `no_directional_evidence`**.

**Scenario F — balanced directional evidence.** Future-topology synthetic case retained from v7: two qualifying agreeing families plus sufficient weak opposing families to bring `|meta_balance| < 0.20`; expected **ABSTAIN `balanced_directional_evidence`**.

**Scenario G — ABSTAIN has no indirect influence.** Add an abstaining sub-calculator to Scenario A and assert all decision-consumption weights, family strengths, meta balance and decision remain bit-identical while the full diagnostic pass still records it.

The canonical fixture SHA and combined manifest digest are frozen in §0.3. Any source-state change requires a new dated preregistration; implementation may not alter this fixture to make tests pass.
...
Frozen and immutable for implementation:

- all Stage A-E mathematics from v7;
- all constants in §4;
- sub-calculators-only contributor universe;
- graph-first order-independent dependency weights;
- BULLISH / BEARISH / ABSTAIN meta output;
- 70 real connected subject identities;
- production-shaped 45-day aggregate / 2-day M1 source shape;
- session-aware market calendar and bar-close `first_observed_at`;
- Build-3 packet/version/scope-linked trust history;
- explicit `decision_time_utc` and derived correctness;
- v9 canonical fixture SHA-256 `74e92fe4a7d38e803da8469262e9339e795d3b9d1f3d7c9594a599560c3e3c4e`;
- combined manifest digest `53e691f0e70308030beda0fd1101f0cd032b51875ecfe2c834aacff507150ba6`.

Executed acceptance evidence before implementation: current Build 20→21→22 path **ABSTAIN 0.075024**; frozen replacement **BULLISH**, two qualifying families, `meta_balance = 1.000000`.

**Next action:** implement T1-T18 against this frozen fixture, confirm the current path fails the reachability expectation, implement §3.1-§3.6 without changing a frozen constant, run focused + full regression, then continue the already-locked sequence in §12.

Production decision logic is still unchanged and RED until that implementation is merged and separately accepted. Formal-forward and live-money authority remain OFF.
