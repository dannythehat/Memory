...
- **0b. Environment key explodes.** 41 cycles produced 41 distinct `environment_key` values; `utc_clock_bucket_15m` and `utc_weekday` are in `GLOBAL_CORE_DIMENSIONS`. `global_core` max N=1 over 570 rows. 100% fallback to `gate_global`. Environment-conditional trust — the point of the programme — never runs.
