...
**Blocker-1 v9 fixture freeze completed 2026-09-22.** The aggregation mathematics remain unchanged from v7/v8 and are now proven executable against the real 15-gate / 70-connected-subcalculator source shape. CI run `35704609730` passed from AIDY evidence head `e4d3a92475189c5d844060d3e7d1b41effe9c8b1`. The frozen fixture SHA-256 is `5d99b3db701df696c5325b1146af5942319db70e05e1c8a2fe130f3e23281960`; combined manifest digest `cf9e44df2965ebbe2b8cbc4e1c21d2e6682dfe978466feb5f3686978bc70e1cd`. From the **same frozen bytes**, the current Build 20→21→22 path abstains with `directional_total=0.152785`, while the frozen replacement reaches **bullish** with `meta_balance=1.000000` and exactly two qualifying independent root families: `price_action=0.331592` and `liquidity_mechanism=0.373330`. The liquidity evidence is produced by the unmodified real builder from a genuine fixture sweep/reclaim, not an injected vote.

This closes the **fixture/reachability design proof only**. It does **not** close blocker 0a in production: AIDY main, deployed logic, formal-forward authority and live-money authority are unchanged. Full frozen specification: `projects/aidy/BLOCKER1_AGGREGATION_PREREGISTRATION.md`; handoff: `projects/aidy/handovers/2026-09-22-blocker1-v9-fixture-freeze.md`.

**Next step is implementation, not more fixture tuning or new data.** Implement the already-frozen Blocker-1 aggregation replacement against the v9 fixture, add it to the production test suite, then re-run Build 23 / prospective shadow acceptance before any promotion reasoning. Environment-cardinality, calibration, PIT-safe H4/D1 rebuild, maintenance-window classification, monitoring/health history and baselines remain separate open repair items.
